...
const Dashboard = (DashboardProps: DashboardProps) => {
  ...
  const [fileAreaMessage, setFileAreaMessage] = useState('');
  const [messages, setMessages] = useState<Message[]>([
    { role: 'system', content: 'Welcome to your AI-powered travel assistant. How can I help you?' }
  ]);
  const [fileAreaMessages, setFileAreaMessages] = useState<Message[]>([
    { role: 'system', content: 'Upload your documents here and I will help review them.' }
  ]);
  ...
  const getGreeting = () => {
    ...
    const handleFilesDrop = (files: File[]) => {
      ...
      setFileAreaMessages(prev => [...prev, userMessage, systemResponse]);
    };

  const cards = [
    {
      title: "Before Departure",
      content: `Get ready for your ${travelReason} trip by preparing all necessary documents and making pre-travel arrangements.`,
      image: getLocationImage(),
      animation: 'animate-float',
      items: ['Passport', 'Visa', 'Travel insurance', 'Health requirements']
    },
    {
      title: "During Travel",
      content: "Navigate through your travels, airport maps and layovers and what you need to be aware of.",
    ...
    {
      title: "After Arrival",
      content: `This section will help you once you arrive to find hotels, register for required events based on your (${travelReason}) journey.`,
  ...
  const handleFileAreaSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-800 dark:text-white flex flex-col">
      ...
      <main className="container mx-auto px-4 py-8 flex flex-col flex-1">
        ...
        {activeTab === 'overview' && (
          <div className='flex flex-col flex-1'>
            <div className="flex-1 flex flex-row space-x-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-center justify-center mb-8">
                        ...
                        {card.items.map((item, itemIndex) => (
                          <div key={itemIndex} className="relative">
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div className="bg-gray-700 rounded-full hover:bg-green-600 px-6 py-3 text-gray-300 hover:text-gray-900 transition-colors duration-300" >
                                <p>{item}</p>
                              </div>
                            </div>
                          </div>
            ...
            <section className="bg-gray-100 dark:bg-gray-800 rounded-xl p-6 shadow-xl mt-auto">
              ...
              <div className="bg-white dark:bg-gray-900 rounded-xl shadow-inner p-4">
                ...
                <form onSubmit={handleSubmit} className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    className="bg-green-500 hover:bg-green-600  text-gray-300 hover:text-gray-900 px-5 py-2 rounded-full transition-colors duration-300 flex items-center"
                  >
                    <PaperAirplaneIcon className="w-7 h-7" />
                  </button>
                </form>
              </div>
              <div className='w-full bottom-0 left-0 mt-4 right-0 text-right text-gray-500 text-sm'>
                powered by <a href="https://www.deepseek.com/" target="_blank" className='text-green-500'>Deepseek r1</a>
              </div>
            </section>
          </div>
        )}
      ...
      {activeTab === 'documents' && (
        ...
        <div className="p-4">
          ...
          <FileDropArea onFilesDrop={handleFilesDrop} />
        </div>
        <section className="bg-gray-100 dark:bg-gray-800 rounded-xl p-6 shadow-xl mt-auto">
          <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-white flex items-center">
            <svg className="w-6 h-6 mr-2 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-inner p-4">
            ...
            <form onSubmit={handleFileAreaSubmit} className="flex gap-2">
              <input
                type="text"
                value={message}
                onChange={(e) => setFileAreaMessage(e.target.value)}
                className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-full focus:outline-none focus:ring-2 focus:ring-green-500 bg-white dark:bg-gray-800 text-gray-800 dark:text-white"
                placeholder="Ask me anything about your trip..."
              />
              <button
